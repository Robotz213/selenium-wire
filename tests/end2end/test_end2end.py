...
def test_google_startup_smoke(driver):
    """Verify Selenium Wire can start Chrome and navigate through its proxy."""
...
def test_update_json_post_request(driver_path, chrome_options, httpbin):
    ...
    chrome_options.add_argument(f"--user-data-dir={chrome_data_dir!s}")
...
def test_upstream_http_proxy_basic_auth(driver_path, chrome_options, httpbin):
    with create_httpproxy(port=8888, auth="test:test") as httpproxy:
        ...
        with create_driver(driver_path, chrome_options, sw_options) as driver:
            ...
            assert (
                "This passed through a authenticated http proxy" in driver.page_source
            )


def test_upstream_http_proxy_basic_auth_empty_pass(
    driver_path, chrome_options, httpbin
):
...
def test_bypass_upstream_proxy_when_target_https(
    driver_path, chrome_options, httpbin, httpproxy
):
...
def test_har(driver_path, chrome_options, httpbin):
    with create_driver(driver_path, chrome_options, {"enable_har": True}) as driver:
        ...
        assert (
            har["log"]["creator"]["comment"]
            == f"Selenium Wire version {seleniumwire.__version__}"
        )
...
def test_in_memory_storage(driver_path, chrome_options, httpbin):
    ...
    with create_driver(driver_path, chrome_options, sw_options) as driver:
        ...
        assert [r.url for r in driver.iter_requests()] == [
            f"{httpbin}/html",
            f"{httpbin}/anything",
        ]
        assert [
            e["request"]["url"] for e in json.loads(driver.har)["log"]["entries"]
        ] == [
            f"{httpbin}/html",
            f"{httpbin}/anything",
        ]
