...
TGeneratedCertId = tuple[bytes | None, tuple[bytes, ...]]  # (common_name, sans)
...
class CertStore:
    ...
    @classmethod
    def from_store(cls, path, basename, key_size, passphrase: bytes | None = None):
    ...
    def create_store(
        ...
        with (
            ...
            open(os.path.join(path, basename + "-ca.pem"), "wb") as f,
        ):
    ...
    def add_cert_file(
        self, spec: str, path: str, passphrase: bytes | None = None
    ) -> None:
    ...
    def get_cert(
        self,
        commonname: bytes | None,
        sans: list[bytes],
        organization: bytes | None = None,
